...
namespace MainAPP.Services.AI
{
    ...
    public sealed class AiWebHost : IDisposable
    {
        ...
        private const string IndexResourceName = "MainAPP.Web.AiIndex.html";

        private static async Task WriteHtmlAsync(HttpListenerResponse response, CancellationToken ct)
        {
            var html = await ReadEmbeddedAsync(IndexResourceName, ct).ConfigureAwait(false) ?? "<h1>AI 页面资源缺失</h1>";
        ...
        private static async Task<string?> ReadEmbeddedAsync(string name, CancellationToken ct)
        {
            try
            {
                using var stream = typeof(AiWebHost).Assembly.GetManifestResourceStream(name);
                if (stream is null)
                {
