...
namespace ConsoleApp.Test
{
    ...
    /// </summary>
    public class CombineModelPool : IDisposable
    {
        ...
        private CombineModelPool(int cpuCount, int gpuCount)
        {
        ...
        /// 使用并发加载来显著减少启动时间。
        /// </summary>
        public static async Task<CombineModelPool> CreateAsync(string modelPath, int cpuCount = 2, int gpuCount = 2)
        {
            ...
            var pool = new CombineModelPool(cpuCount, gpuCount);
            ...
            try
            {
                ...
                for (int i = 0; i < cpuCount; i++)
                {
                    tasks.Add(Task.Run(() =>
                    {
                        var model = CreateYoloModel(modelPath, DeviceType.CPU);
                        pool._cpuModels.Add(model);
                    }));
                }
                ...
                for (int i = 0; i < gpuCount; i++)
                {
                    tasks.Add(Task.Run(() =>
                    {
                        var model = CreateYoloModel(modelPath, DeviceType.GPU);
                        pool._gpuModels.Add(model);
                    }));
                }
        ...
        private static Yolo CreateYoloModel(string modelPath, DeviceType deviceType)
        {
            // 可以在此处将配置参数化，而不是硬编码
            return new Yolo(new YoloOptions
            {
                ExecutionProvider = new OpenVinoExecutionProvider(
                    ...
                    new OpenVino
                    {
                        ...
                        CachePath = "OpenVinoCache", // 建议后续改为可配置
        ...
        {
            if (_disposed) throw new ObjectDisposedException(nameof(CombineModelPool));
            ...
            {
                throw new ObjectDisposedException(nameof(CombineModelPool));
            }
        }
